const hexToRgb = input => {
  ...
  if (
    !hexRegex.test(inputTemp) ||
    (inputTemp.length !== 3 && inputTemp.length !== 6)
  ) {
    throw new Error('input is not a valid hex color.');
  }
  if (inputTemp.length === 3) {
    const first = inputTemp[0];
    const second = inputTemp[1];
    const last = inputTemp[2];
    inputTemp = first + first + second + second + last + last;
  }
  inputTemp = inputTemp.toUpperCase(inputTemp);
  const first = inputTemp[0] + inputTemp[1];
  const second = inputTemp[2] + inputTemp[3];
  const last = inputTemp[4] + inputTemp[5];
